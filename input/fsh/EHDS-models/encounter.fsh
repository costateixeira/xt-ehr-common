...
* admission.admitter 0..1 EHDSHealthProfessional "Admitting healthcare professional"
* admission.admittingOrganization 0..1 EHDSOrganisation "Admitting healthcare professional"

* admission.admitSourceType 0..1 CodeableConcept "From where the patient was admitted (e.g. physician referral, transfer)."
  * ^binding.description = "hl7:admit-source"
  * ^binding.strength = #preferred
* admission.admitSource 0..1 EHDSOrganisation "From where the patient was admitted (e.g. physician referral, transfer)."
  * ^binding.description = "hl7:admit-source"
  * ^binding.strength = #preferred
* admission.referringProfessional 0..1 EHDSHealthProfessional "Referring Healthcare Professional"


// Extended admission details
* admission.legalStatus 0..1 CodeableConcept "Legal status/situation at admission (indicates the basis on which the patient is staying in a healthcare organisation)." "Legal status can be either voluntary or involuntary, however the legal status is always determined by a court. A patient can also receive healthcare based on a forensic status. (voluntary, involuntary, admission by legal authority)."
...
// Elements inherited from EHDSEncounterCore:
// - priority
// - actualPeriod
// - admission (reason[x], reasonComment)
// - discharge (destinationType, destinationLocation[x])
// - location (period, organisationPart[x])

...
* timing 0..1 Base "When medication should be administered (period, time of day, frequency, etc)"
  ...
  * repeat 0..1 Base "Repetition of the administration."
    ...
    * count 0..1 Base "Number of times to repeat, exact or range"
      * count 0..1 integer "Number of times (e.g 'once', '10 times')"
      * countMax 0..1 integer "Maximum number of times (e.g 'maximum 10 times')"
    * duration 0..1 Base "Duration of one administration, exact or range"
...
Parent: EHDSDosagingCore
Title: "Dosaging model"
Description: "Logical model for usage instructions for administring the requested product. Based on FHIR Dosage complex data type. When implemented, this model may be reduced significantly according to the specific use case."
Characteristics: #can-be-target

// Elements inherited from EHDSDosagingCore:
// - text, renderedDescription, additionalInstruction, patientInstruction
// - doseAndRate (type, dose[x], rate[x])
// - timing (event, code, repeat with all sub-elements)
// - asNeeded, asNeededFor, bodySite, routeOfAdministration, methodOfAdministration
// - maxDose (maxDosePerPeriod, maxDosePerAdministration, maxDosePerLifetime)
